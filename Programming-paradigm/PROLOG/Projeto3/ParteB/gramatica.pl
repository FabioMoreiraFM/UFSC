...
comando --> [listar], {listar}.

% ---- Posição atual da tartaruga --------
comando --> [postar], {postar}.

% ---- Muda o ângulo e vai para a frente ----
comando --> [girafrente], [N], [M], {atom_number(N,X), giradireita(X),
				   atom_number(M,Y), parafrente(Y)}.
